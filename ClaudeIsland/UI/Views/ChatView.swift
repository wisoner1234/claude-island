...
struct ChatView: View {
    ...
    /// Can send messages if session is in tmux (with tty) or has a PID (non-tmux terminal messaging)
    private var canSendMessages: Bool {
        (session.isInTmux && session.tty != nil) || (!session.isInTmux && session.pid != nil)
    }

    private var inputBar: some View {
        HStack(spacing: 10) {
            TextField(canSendMessages ? "Message Claude..." : "No session available", text: $inputText)
    ...
    private func sendToSession(_ text: String) async {
        ...
        if session.isInTmux, let tty = session.tty {
            // Tmux: use send-keys via ToolApprovalHandler
            if let target = await findTmuxTarget(tty: tty) {
                let ok = await ToolApprovalHandler.shared.sendMessage(text, to: target)
                if !ok {
                    sendError = "Failed to send via tmux"
                }
            } else {
                sendError = "Tmux pane not found"
            }
        } else if let pid = session.pid {
            // Non-tmux: use CGEvent-based TerminalMessenger
            let ok = await TerminalMessenger.shared.sendMessage(text, toSessionPid: pid)
            if !ok {
                sendError = "Could not reach terminal (check Accessibility permission)"
            }
        }
    }
...
struct ChatInteractivePromptBar: View {
    ...
    var body: some View {
        HStack(spacing: 10) {
            ...
            // Input field or Terminal button
            if canSendMessages {
